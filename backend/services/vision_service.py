...
class JointGuardStateEngine:
    ...
    def _finalize_track_verdict(self, track: JointTrack):
        ...
        if n_samples >= min(3, self.target_frames):
            # If the track's temporal average clearly leans DAMAGE by the time it passes:
            if avg_d > avg_h:
                ...
                # Exact tie in average probabilities: break tie with sample counts
            ...
            if sys.platform.startswith("win"):
                ...
                if cap and cap.isOpened():
                    ...
                    if ret:
                        ...
                        print(f"[VISION SERVICE] Successfully opened camera index {idx} (CAP_DSHOW)")
                        return True
                    cap.release()
